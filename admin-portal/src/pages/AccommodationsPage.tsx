...
export default function AccommodationsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const data = await listAccommodations(page, 10, search);
      setItems(data.items);
      setTotal(data.total);
      setPages(data.pages);
    } finally {
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{total} accommodations tracked</p>
        </div>
        <Link
      ...
      <form onSubmit={handleSearch} className="mb-4 flex gap-2">
        <input
          ...
          placeholder="Search hotel, city, pilgrim..."
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-500">No accommodations found.</td></tr>
              ) : (
                items.map((a) => (
                  <tr key={a.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{new Date(a.check_in).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-gray-600">{new Date(a.check_out).toLocaleDateString()}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={`/accommodations/${a.id}`} className="text-gray-500 hover:text-gray-700 text-xs">View</Link>
                        <Link to={`/accommodations/${a.id}/edit`} className="text-emerald-600 hover:text-emerald-800 text-xs">Edit</Link>
                      </div>
                    </td>
                  </tr>
                ))

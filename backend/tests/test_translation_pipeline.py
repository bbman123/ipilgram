...
Tests all supported languages (English, Arabic, French, Urdu, Hausa, Yoruba, Hindi).
Verifies:
  - Translation caching by AnnouncementID+Language
  - TTS generation from translated text
...
from app.services.tts import generate_audio, LANGUAGE_MAP
...
class TestStructuredLogging:
    """Test that specific log messages appear in the right places."""

    def test_translation_cache_hit_log_message(self):
        """Verify 'Translation cache hit' log message format."""
        expected = "Translation cache hit: announcement_id=%d, language=%s"
        # This is just a format validation - actual log testing is done in TestLogging

    def test_announcement_translated_log_message(self):
        """Verify 'Announcement X translated to Y' log message format."""
        expected = "Announcement %d translated to %s: title_length=%d, message_length=%d"
        # This is just a format validation

    def test_audio_cache_hit_log_message(self):
        """Verify 'Audio cache hit' log message format."""
        expected = "Audio cache hit: language=%s, file=%s"
        # This is just a format validation

    def test_tts_generated_log_message(self):
        """Verify 'TTS generated' log message format."""
        expected = "TTS generated: language=%s, file=%s, size=%d bytes"
        # This is just a format validation
